...
def get_elevation_data(trip_id, osm_data, elevation_sampler, engine, first_sample_distance = 10, end_sample_distance = 100, brunnel_filter_length=10, interpolated = True, adjust_window_size = 12, std_thresh = 3, sub_factor = 3, clip = 20, smooth_window_size=301, poly_order=3, degrees=False, smooth_after_resampling=True, window_size_2=5,poly_order_2=1, mode="nearest"):
    ...
    x_coords, y_coords, distances, elevation = elevation_sampler.elevation_profile(trip_geom, distance=first_sample_distance, interpolated = interpolated)
    
    ele_brunnel = elevation_sampler.interpolate_brunnels(elevation, distances, brunnels, first_sample_distance)

    ele_adjusted = elevation_sampler.adjust_forest_height(ele_brunnel,window_size = adjust_window_size, std_thresh = std_thresh, sub_factor = sub_factor, clip = clip)
    
    ele_smoothed =  elevation_sampler.smooth_ele(ele_adjusted, window_size=smooth_window_size, poly_order=poly_order, mode=mode)
    ...
    if smooth_after_resampling:
        elevation_100 = elevation_sampler.smooth_ele(elevation_100, window_size=window_size_2, poly_order=poly_order_2,mode=mode)
...
def get_timetable(trip_id, engine, min_stop_duration=30, round_int = True):
    ...
    sql = """
    select geo_trips.trip_headsign, geo_stop_times.stop_sequence, geo_stop_times.arrival_time, geo_stop_times.departure_time, geo_stops.stop_name,
    ST_LineLocatePoint(ST_Transform(geo_shape_geoms.geom,25832),ST_Transform(ST_SetSRID(ST_MakePoint(stop_lon,stop_lat),4326),25832)) * ST_length(ST_Transform(geo_shape_geoms.geom,25832)) as dist
    ...
    driving_time = time_table.arrival_time[1:].dt.total_seconds().values-time_table.departure_time[:-1].dt.total_seconds().values
...
def trip_title_to_filename(title):
    special_char_map = {ord('ä'):'ae', ord('ü'):'ue', ord('ö'):'oe', ord('ß'):'ss', ord('('):'',  ord(')'):'',  ord(' '):'_'}
...
def write_input_sheet(trip_title, timetable, electrification, maxspeed, inclination):
    ...
    # electrification
    electrification[["start_dist","electrified"]].to_excel(writer, sheet_name='Timetable and limits', index=False,header=False,startrow=current_row)
    ...
    # maxspeed
    maxspeed[["start_dist","maxspeed"]].to_excel(writer, sheet_name='Timetable and limits', index=False,header=False,startrow=current_row)
...
def get_trip_data(trip_id, engine, elevation_sampler, crs=25832):
    ...
    osm_data = sql_get_osm(trip_id, engine, 25832)
    ...
    inclination = get_elevation_data(trip_id, osm_data, elevation_sampler, engine)
